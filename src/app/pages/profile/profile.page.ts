...
export class ProfilePage implements OnInit {
  ...
  constructor(
    ...
    this.contentService.currentUser.subscribe(user => {
      ...
      this.generateContentHistory(user.history);
    });
  }
  ...
  generateStatistics(history: History[]) {
    ...
    history.forEach(h => {
      switch (this.commonService.checkWeek(h.date)) {
        case 1:
          ...
          newData[index1] += this.commonService.calculateTrophy(h.point);
        ...
        case 0:
          ...
          newData[index2] += this.commonService.calculateTrophy(h.point);
          break;
        default:
          break;
      }
  ...
  generateContentHistory(history: History[]) {
    this.historyContent = [];
    history.forEach(async h => {
      this.historyContent.push(await this.contentService.getContentsById(h.id));
    });
  }
